...
public class PredictionVerificationResult {
	...
	public synchronized void newMatch(MatchForVerification match, boolean predictable, double probability, boolean predicted, boolean withPrice, boolean beatingPrice, boolean profitable, double stake, double return_) {
		result.newMatch(predictable, predicted, withPrice, beatingPrice, profitable, stake, return_);
		Surface surface = match.surface;
		if (surface != null)
			surfaceResults.computeIfAbsent(surface, s -> new PredictionResult(config)).newMatch(predictable, predicted, withPrice, beatingPrice, profitable, stake, return_);
		TournamentLevel level = match.level;
		if (level != null)
			levelResults.computeIfAbsent(level, s -> new PredictionResult(config)).newMatch(predictable, predicted, withPrice, beatingPrice, profitable, stake, return_);
		if (predictable) {
			probability = probability >= 0.5 ? probability : 1.0 - probability;
			probability = Math.round(probability * 20) / 20.0;
			probabilityRangeResults.computeIfAbsent(probability, s -> new PredictionResult(config)).newMatch(predictable, predicted, withPrice, beatingPrice, profitable, stake, return_);
		}
		Range<Integer> rankRange = rankRange(nullsLastMin(match.winnerRank, match.loserRank));
		if (rankRange != null)
			rankRangeResults.computeIfAbsent(rankRange, s -> new PredictionResult(config)).newMatch(predictable, predicted, withPrice, beatingPrice, profitable, stake, return_);
	}
