package org.strangeforest.tcb.stats.model.records;

public class RankingATPCategory extends RankingCategory {

	private static final String ADJUSTMENT = " (adjusted by factor 1.9 before 2009)";

	public RankingATPCategory() {
		super("ATP Ranking");
		registerRanking("ATP", N_A);
		register(mostPoints("ATPPoints", "Most ATP Points" + ADJUSTMENT, "player_best_rank_points", "best_rank_points_adjusted", "best_rank_points_adjusted_date", "ATP Points"));
		register(mostEndOfSeasonPoints("EndOfSeasonATPPoints", "Highest End of Season ATP Points" + ADJUSTMENT, "player_year_end_rank", "adjust_atp_rank_points(year_end_rank_points, season_start(season))", "ATP Points"));
		register(greatestPointsDifferenceBetweenNo1andNo2(
			"ATPPointsNo1No2Difference", "ATP Points Difference Between No. 1 and No. 2" + ADJUSTMENT, "player_ranking", "rank_points",
			"adjust_atp_rank_points(r1.rank_points - r2.rank_points, r1.rank_date)", "adjust_atp_rank_points(r1.rank_points, r1.rank_date)", "adjust_atp_rank_points(r2.rank_points, r1.rank_date)",
			"numeric", null, "Points", "Points Diff."
		));
		register(greatestPointsDifferenceBetweenNo1andNo2(
			"ATPPointsNo1No2DifferencePct", "ATP Points Pct. Difference Between No. 1 and No. 2", "player_ranking", "rank_points",
			"round(100 * (r1.rank_points - r2.rank_points) / r2.rank_points)::INTEGER", "r1.rank_points", "r2.rank_points",
			null, "pct", "Points", "Points Pct. Diff."
		));
	}
}
