	...
	private static Record resultStreak(RecordDomain domain, String id, String name, String nameSuffix, String resultCondition, String partition, String resultUrlParam) {
		return new Record<>(
			...
			StreakRecordDetail.class, (playerId, recordDetail) -> {
				String url = format("/playerProfile?playerId=%1$d&tab=tournaments%2$s&result=%3$s&fromDate=%4$td-%4$tm-%4$tY&toDate=%5$td-%5$tm-%5$tY", playerId, domain.urlParam, resultUrlParam, recordDetail.getStartDate(), recordDetail.getEndDate());
