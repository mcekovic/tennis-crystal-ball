...
public class EloRankingCategory extends RankingCategory {
	...
	public EloRankingCategory(RecordDomain domain) {
		...
		register(mostPoints(domain.id + "EloRating", "Highest " + suffix(domain.name, " ") + "Elo Rating", "player_best_elo_rating", "best_" + ratingColumn, "best_" + ratingColumn + "_date", "Elo Rating"));
		if (domain == ALL)
