...
public class WinningStreaksCategory extends RecordCategory {
	...
	public WinningStreaksCategory() {
		...
		register(winningStreak(ALT_FINALS, N_A, "_level"));
		register(winningStreak(ALL_FINALS, N_A, "_level"));
	...
	private static Record winningStreakVs(RecordDomain domain, String tableName) {
		return winningStreakRecord("WinningStreakVs" + domain.id, "Winning Streak Vs " + domain.name, "_vs_" + tableName, N_A, true, false, domain.urlParam);
	}
	...
	private static Record tournamentWinningStreak(RecordDomain domain, String tableName, String condition) {
		if (condition == null)
			condition = domain.condition;
		return winningStreakRecord(domain.id + "TournamentWinningStreak", "Winning Streak at Single " + suffix(domain.name, " ") + "Tournament", tableName, condition, true, true, domain.urlParam);
	}

	private static Record winningStreakRecord(String id, String name, String tableName, String condition, boolean useMaterializedView, boolean forTournament, String urlParam) {
		return new Record<>(
			id, name,
			/* language=SQL */
			(useMaterializedView ? "" : playerWinStreak(condition)) +
			"SELECT player_id, s.win_streak AS value" + (forTournament ? ", fe.tournament_id" : "") +",\n" +
			"  fe.season AS start_season, fm.date AS start_date, fe.tournament_event_id AS start_tournament_event_id, fe.name AS start_tournament, fe.level AS start_level,\n" +
			"  le.season AS end_season, lm.date AS end_date, le.tournament_event_id AS end_tournament_event_id, le.name AS end_tournament, le.level AS end_level\n" +
