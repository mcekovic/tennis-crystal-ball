function showRankingChart(chartData, elementId, width, bySeason, pointCount, isRank, useLogScale, legendPosition) {
	showChart(chartData, elementId, width, bySeason, pointCount, isRank ? -1 : 1, isRank ? 1 : undefined, useLogScale, legendPosition);
}

function showChart(chartData, elementId, width, bySeason, pointCount, vDir, vMin, useLogScale, legendPosition) {
	if (chartData != undefined) {
		var options = {
			...
			vAxis: {direction: vDir, viewWindow: {min: vMin}, logScale: useLogScale},
