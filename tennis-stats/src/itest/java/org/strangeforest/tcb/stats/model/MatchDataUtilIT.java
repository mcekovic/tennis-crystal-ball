...
class MatchDataUtilIT {
	...
	void testProbabilityTransformer() {
		...
		for (var p = 0.0; p <= 1.0; p += 0.01) {
			var pBestOf3 = bestOf3MatchProbability(p);
			var pBestOf5 = bestOf5MatchProbability(p);
			var pBestOf3FromMixed = bestOf3FromMixedProbability(p);
			var pBestOf5FromMixed = bestOf5FromMixedProbability(p);
